...
def main():
    ...
    for setting_cond in SETTING_CONDS:
        for balanced_cond in BALANCED_CONDS:
            ...
            y_true = pool_trues(y_true_list)  # check all trues are aligned
            for strategy in POOL_STRATEGIES:
                ...
                report_path = os.path.join(out_dir, '%s_report.json' % strategy)
...
def generate_ensemble_predictions(y_score_list: list[list[float]],
    ...
    for model_scores in transposed_scores:
        ...
        # Pooled prediction is 1 if scores of all models are > threshold
        if strategy == 'unanimity':
            zipped = zip(model_scores, thresh_optim_list)
            all_votes_pos = all(score >= thresh for score, thresh in zipped)
            y_scores.append(int(all_votes_pos))  # whether all models voted for pos
            pooled_threshold = 0.5  # any number between 0 and 1 will do
        
        # Pooled prediction is 1 if scores of a majority of models are > threshold
        elif strategy == 'majority':
            zipped = zip(model_scores, thresh_optim_list)
            n_votes_pos = sum(score >= thresh for score, thresh in zipped)
            y_scores.append(int(n_votes_pos > (len(thresh_optim_list) / 2)))
            pooled_threshold = 0.5  # any number between 0 and 1 will do
